    ...
    confirmButton.addEventListener('click', async () => {
        ...
        try {
            ...
            if (response.ok && data.tableId && data.tableNumber) {
                console.log("All good here "); // Debugging the URL
                // Redirect to reservation form with table details
                const reservationUrl = `/reservation-form?tableId=${data.tableId}&tableNumber=${data.tableNumber}&date=${selectedDate}&time=${selectedTimeValue}`;
